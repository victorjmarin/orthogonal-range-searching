 ...
 * /geometry/partitioning/utilities/AVLTree.html. Instead of using the Comparable<T>
 * interface to compare nodes, it allows the use of any Comparator<T>.
 */
 ...
 * <p>
 * The purpose of this class is to sort elements while allowing duplicate elements (i.e.
 * such that {@code a.equals(b)} is true). The {@code SortedSet} interface does not allow
 * this, so a specific class is needed. Null elements are not allowed.
 * </p>
 * <p>
 * Since the {@code equals} method is not sufficient to differentiate elements, the
 * {@link #delete delete} method is implemented using the equality operator.
 * </p>
 * <p>
 * In order to clearly mark the methods provided here do not have the same semantics as
 * the ones specified in the {@code SortedSet} interface, different names are used
 * ({@code add} has been replaced by {@link #insert insert} and {@code remove} has been
 * replaced by {@link #delete delete}).
 * </p>
 * <p>
 * This class is based on the C implementation Georg Kraml has put in the public domain.
 * Unfortunately, his <a href="www.purists.org/georg/avltree/index.html">page</a> seems
...
public class AVLTree<T> {
     ...
     * <p>
     * The element is deleted only if there is a node {@code n} containing exactly the
     * element instance specified, i.e. for which {@code n.getElement() == element}. This
     * is purposely <em>different</em> from the specification of the {@code java.util.Set}
     * {@code remove} method (in fact, this is the reason why a specific class has been
     * developed).
     * </p>
     * 
     * @param element
     ...
     * 
     * @return the tree node containing the smallest element in the tree or null if the
     *         tree is empty
     * @see #getLargest
     * @see #getNotSmaller
     * @see #getNotLarger
     ...
     * 
     * @return the tree node containing the largest element in the tree or null if the
     *         tree is empty
     * @see #getSmallest
     * @see #getNotSmaller
     * @see #getNotLarger
     ...
     * @return the tree node containing the smallest element not smaller than the
     *         reference object or null if either the tree is empty or all its elements
     *         are smaller than the reference object
     * @see #getSmallest
     * @see #getLargest
     * @see #getNotLarger
     * @see Node#getPrevious
     ...
     * @param reference
     *            reference object (may not be in the tree)
     * @return the tree node containing the largest element not larger than the reference
     *         object (in which case the node is guaranteed not to be empty) or null if
     *         either the tree is empty or all its elements are larger than the reference
     *         object
     * @see #getSmallest
     * @see #getLargest
     * @see #getNotSmaller
     ...
     * <p>
     * AVL tree nodes implement all the logical structure of the tree. Nodes are created
     * by the {@link AVLTree AVLTree} class.
     * </p>
     * <p>
     * The nodes are not independent from each other but must obey specific balancing
     * constraints and the tree structure is rearranged as elements are inserted or
     * deleted from the tree. The creation, modification and tree-related navigation
	 ...
	 * 
	 * @return the tree node containing the smallest element in the tree rooted at
	 *         this node or null if the tree is empty
	 * @see #getLargest
	 */
	Node getSmallest() {
	    Node node = this;
	 ...
	 * 
	 * @return the tree node containing the largest element in the tree rooted at this
	 ...
	 * 
	 * @return node containing the next smaller or equal element or null if there is
	 *         no smaller or equal element in the tree
	 * @see #getNext
	 */
	 ...
	 * 
	 * @return node containing the next larger or equal element (in which case the
	 *         node is guaranteed not to be empty) or null if there is no larger or
	 ...
	 * <p>
	 * The skew factor are not updated by this method, they <em>must</em> be updated
	 * by the caller
	 * </p>
	 */
