 ...
 * http://commons.apache.org/proper/commons-math/javadocs/api-3.3/org/apache/commons/math3
 * /geometry/partitioning/utilities/AVLTree.html. Instead of using the Comparable<T> interface to
 * compare nodes, it allows the use of any Comparator<T>.
 */
 ...
 * <p>
 * The purpose of this class is to sort elements while allowing duplicate elements (i.e. such that
 * {@code a.equals(b)} is true). The {@code SortedSet} interface does not allow this, so a specific
 * class is needed. Null elements are not allowed.
 * </p>
 * <p>
 * Since the {@code equals} method is not sufficient to differentiate elements, the {@link #delete
 * delete} method is implemented using the equality operator.
 * </p>
 * <p>
 * In order to clearly mark the methods provided here do not have the same semantics as the ones
 * specified in the {@code SortedSet} interface, different names are used ({@code add} has been
 * replaced by {@link #insert insert} and {@code remove} has been replaced by {@link #delete
 * delete}).
 * </p>
 * <p>
 * This class is based on the C implementation Georg Kraml has put in the public domain.
 * Unfortunately, his <a href="www.purists.org/georg/avltree/index.html">page</a> seems not to exist
 * any more.
 * </p>
 *
...
public class AVLTree<T> {
   ...
   * <p>
   * The element is deleted only if there is a node {@code n} containing exactly the element
   * instance specified, i.e. for which {@code n.getElement() == element}. This is purposely
   * <em>different</em> from the specification of the {@code java.util.Set} {@code remove} method
   ...
   * 
   * @return the tree node containing the smallest element in the tree or null if the tree is empty
   * @see #getLargest
   * @see #getNotSmaller
   * @see #getNotLarger
   ...
   * 
   * @return the tree node containing the largest element in the tree or null if the tree is empty
   * @see #getSmallest
   * @see #getNotSmaller
   * @see #getNotLarger
   * @see Node#getPrevious
   ...
   * @return the tree node containing the smallest element not smaller than the reference object or
   *         null if either the tree is empty or all its elements are smaller than the reference
   *         object
   * @see #getSmallest
   * @see #getLargest
   * @see #getNotLarger
  ...
  public Node getNotSmaller(final T reference) {
    ...
    for (Node node = top; node != null;) {
      if (comparator.compare(node.element, reference) < 0) {
        if (node.right == null) {
          return candidate;
        }
        node = node.right;
   ...
   * 
   * @param reference reference object (may not be in the tree)
   * @return the tree node containing the largest element not larger than the reference object (in
   *         which case the node is guaranteed not to be empty) or null if either the tree is empty
   *         or all its elements are larger than the reference object
   * @see #getSmallest
   * @see #getLargest
   * @see #getNotSmaller
   * @see Node#getPrevious
  ...
  public void setTop(final Node n) {
    top = n;
  }
  ...
  public List<T> searchInRange(final T loRef, final T hiRef) {
    ...
    final AVLTree<T>.Node u = getNotSmaller(loRef);
    final AVLTree<T>.Node v = getNotLarger(hiRef);
    if (u == null || v == null)
   ...
   * <p>
   * AVL tree nodes implement all the logical structure of the tree. Nodes are created by the
   * {@link AVLTree AVLTree} class.
   * </p>
   * <p>
   * The nodes are not independent from each other but must obey specific balancing constraints and
   * the tree structure is rearranged as elements are inserted or deleted from the tree. The
   * creation, modification and tree-related navigation methods have therefore restricted access.
   * Only the order-related navigation, reading and delete methods are public.
   * </p>
   * 
   * @see AVLTree
   */
  public class Node {
     ...
     * 
     * @return the tree node containing the smallest element in the tree rooted at this node or null
     ...
     * 
     * @return the tree node containing the largest element in the tree rooted at this node or null
     *         if the tree is empty
     * @see #getSmallest
     */
    Node getLargest() {
     ...
     * 
     * @return node containing the next smaller or equal element or null if there is no smaller or
     ...
     * 
     * @return node containing the next larger or equal element (in which case the node is
     *         guaranteed not to be empty) or null if there is no larger or equal element in the
     *         tree
     * @see #getPrevious
     */
    public Node getNext() {
    ...
    boolean insert(final T newElement) {
      if (comparator.compare(newElement, this.element) < 0) {
